...
def _mk(rule_id: str, rule_name: str, country: str, entity: str, metrics: dict,
        threshold: str, evidence: str, action: str) -> Alarm:
    return Alarm(
        id=f"{rule_id}-{country}-{entity}", rule_id=rule_id, rule_name=rule_name,
        severity=cfg.SEVERITY[rule_id], country=country, entity=entity,
        metrics=metrics, threshold=threshold, evidence=evidence,
...
def rule_budget_consumption() -> list[Alarm]:
    ...
    for r in hot.itertuples():
        out.append(_mk(
            "A1", "Presupuesto cerca de agotarse", r.country, r.category,
            ...
            (f"{r.country}: {r.category} ({r.cost_type}) lleva {r.consumption_pct:.1f}% "
             f"del presupuesto anual consumido a junio, con {r.months_remaining} meses restantes."),
            "Congelar compromisos no esenciales y solicitar reasignación presupuestaria antes del Q3.",
        ))
    return out
...
def _a1_flagged() -> set[tuple[str, str]]:
    bc = fin.budget_consumption()
    hot = bc[bc["consumption_pct"] > cfg.TH_BUDGET_CONSUMPTION_PCT]
    return {(r.country, r.category) for r in hot.itertuples()}
...
def rule_opex_runrate() -> list[Alarm]:
    ...
    for r in hot.itertuples():
        if (r.country, r.category) in skip:
            continue
        out.append(_mk(
            "A2", "Run-rate proyecta sobregasto OPEX", r.country, r.category,
            ...
            (f"{r.country}: {r.category} proyecta cerrar el año en {r.run_rate_fy_pct:.1f}% "
             f"del presupuesto si mantiene el ritmo actual (consumo YTD {r.consumption_pct:.1f}%)."),
            "Renegociar tarifas/contratos y definir plan de mitigación con el país antes del cierre de Q3.",
        ))
    return out
...
def rule_capex_overrun() -> list[Alarm]:
    ...
    hot = bc[(bc["committed_pct"] > cfg.TH_CAPEX_COMMITTED_PCT)
             | (bc["run_rate_fy_pct"] > cfg.TH_RUNRATE_PCT)]
    for r in hot.itertuples():
        if (r.country, r.category) in skip:
            continue
        out.append(_mk(
            "A3", "CAPEX comprometido/proyectado sobre presupuesto", r.country, r.category,
            {"committed_pct": r.committed_pct, "run_rate_fy_pct": r.run_rate_fy_pct,
             "annual_budget_usd": r.annual_budget_usd, "ytd_spent_usd": r.ytd_actual_usd},
            ...
            (f"{r.country}: {r.category} tiene {r.committed_pct:.1f}% del presupuesto anual "
             f"ya comprometido (POs emitidas) y un run-rate de gasto de {r.run_rate_fy_pct:.1f}%."),
            "Revisar el plan de despliegue con Ingeniería: repriorizar fases o ampliar presupuesto vía CAPEX committee.",
        ))
    return out
...
def rule_margin_compression() -> list[Alarm]:
    ...
    for (country, segment), grp in gm.groupby(["country", "segment"]):
        if len(grp) == 2 and (grp["variance_pp"] <= -cfg.TH_MARGIN_GAP_PP).all():
            ...
            out.append(_mk(
                "A4", "Compresión de margen bruto vs objetivo", country, segment,
                ...
                (f"{country} {segment}: margen bruto {last['gm_pct']:.1f}% vs objetivo "
                 f"{last['gm_target_pct']:.1f}% ({last['variance_pp']:+.1f}pp) por segundo mes consecutivo."),
                "Revisar costos directos del segmento (contratos de contenido/interconexión) y evaluar repricing.",
            ))
    return out
...
def rule_revenue_decline() -> list[Alarm]:
    ...
    for (country, segment), grp in tr.groupby(["country", "segment"]):
        grp = grp.sort_values("month")
        last2 = grp["mom_pct"].tail(2)
        if len(last2) == 2 and (last2 <= -cfg.TH_REVENUE_DECLINE_PCT).all():
            out.append(_mk(
                "A5", "Caída sostenida de ingresos", country, segment,
                ...
                "Analizar churn/competencia en el mercado y activar plan comercial de retención.",
            ))
    return out
...
def rule_cost_spike() -> list[Alarm]:
    ...
    for (country, category), grp in op.groupby(["country", "category"]):
        grp = grp.sort_values("month")
        if len(grp) < 2:
            continue
        ...
        if mom >= cfg.TH_COST_SPIKE_MOM_PCT:
            out.append(_mk(
                "A6", "Pico de costo mensual", country, category,
                {"mom_pct": round(mom, 1), "last_month_usd": round(last, 0),
                ...
                "Validar si es un one-off (tarifa/estacionalidad) y ajustar el forecast de OPEX del país.",
            ))
    return out
...
def rule_collections() -> list[Alarm]:
    ...
    for country in cfg.COUNTRIES:
        ...
        if dso_breach or coll_breach:
            out.append(_mk(
                "A7", "Deterioro de cobranza / DSO", country, "B2B",
                {"dso_days_now": current, "dso_days_prev3_avg": round(prev_avg, 1),
                ...
                (f"{country} B2B: DSO subió a {current:.0f} días (promedio previo "
                 ...
                 f"de lo facturado."),
                "Escalar gestión de cobranza con los clientes morosos principales y revisar provisiones de incobrables.",
            ))
    return out
...
def rule_overdue_client() -> list[Alarm]:
    ...
    hot = top[(top["overdue_over60_usd"] > 0)
              & (top["overdue_vs_monthly_billing_pct"] > cfg.TH_OVERDUE_VS_BILLING_PCT)]
    for r in hot.itertuples():
        out.append(_mk(
            "A8", "Cliente B2B con mora crítica", r.country, r.client_id,
            {"client_name": r.client_name, "sector": r.sector,
             "overdue_over60_usd": r.overdue_over60_usd,
             "overdue_total_usd": r.overdue_usd,
             "max_overdue_days": int(r.max_overdue_days),
             "monthly_billing_usd": r.monthly_billing_usd},
            f"mora > 60 días superior al {cfg.TH_OVERDUE_VS_BILLING_PCT:.0f}% de su facturación mensual",
            (f"{r.client_name} ({r.country}): ${r.overdue_over60_usd / 1e6:.1f}M vencidos "
             f"a más de 60 días ({r.overdue_vs_monthly_billing_pct:.0f}% de su facturación "
             f"mensual, máx. {int(r.max_overdue_days)} días)."),
            "Reunión ejecutiva de cobranza, plan de pagos formal y evaluar suspensión parcial de servicios no críticos.",
        ))
    return out
...
def rule_concentration() -> list[Alarm]:
    out = []
    top = fin.top_overdue_clients(limit=10)
    hot = top[(top["share_of_country_b2b_pct"] > cfg.TH_CONCENTRATION_PCT)
              & (top["overdue_usd"] > 0)]
    for r in hot.itertuples():
        out.append(_mk(
            "A9", "Concentración de ingresos B2B en cliente moroso", r.country, r.client_id,
            ...
            (f"{r.client_name} representa {r.share_of_country_b2b_pct:.1f}% del ingreso B2B "
             f"de {r.country} y mantiene saldos vencidos — riesgo combinado de ingreso y crédito."),
            "Diversificar cartera B2B del país y condicionar renovaciones a regularización de pagos.",
        ))
    return out


RULES = [
    rule_budget_consumption, rule_opex_runrate, rule_capex_overrun,
    rule_margin_compression, rule_revenue_decline, rule_cost_spike,
    rule_collections, rule_overdue_client, rule_concentration,
]
...
if __name__ == "__main__":
    ...
    print("\nmanifest match:", "OK" if expected == got else
          f"MISMATCH missing={expected - got} extra={got - expected}")
